...
class UFF:
    ...
    **Data-set 55 (data at nodes)**:
    ...
    def read_sets(self, setn=None):
        ...
        if not self.file_exists():
            raise UFFException(
                "Cannot read from a non-existing file: " + self._fileName
            )
        if not self._refreshed:
        ...
        try:
            ...
            raise UFFException(
                "Error when reading " + str(ii) + "-th data-set: " + msg.value
            )
        except:
    ...
    def _read_set(self, n):
        ...
        try:
            ...
            raise UFFException(
                "Cannot access the file: " + self._fileName + " to read from."
            )
        else:
            try:
                ...
                if self._setTypes[int(n)] == 58:
                    blockData = fh.read(
                        ei - si + 1
                    )  # decoding is handled later in _extract58
                else:
    ...
    def _write_set(self, dset, mode="add"):
        ...
        if mode.lower() == "overwrite":
            ...
            try:
                ...
                raise UFFException(
                    "Cannot access the file: " + self._fileName + " to write to."
                )
        elif mode.lower() == "add":
            # add (append) mode
            try:
        ...
        try:
            ...
            try:
                ...
                fh.close()
                raise UFFException(
                    "Data-set's dictionary is missing the required 'type' key"
                )
            # handle nan or inf
    ...
    def _write15(self, fh, dset):
        ...
        try:
            ...
            for ii in range(0, n):
                fh.write(
                    "%10i%10i%10i%10i%13.5e%13.5e%13.5e\n"
                    % (
                        dset["node_nums"][ii],
                        dset["def_cs"][ii],
                        dset["disp_cs"][ii],
    ...
    def _write82(self, fh, dset):
        ...
        try:
            # handle optional fields
            dset = self._opt_fields(dset, {"id": "NONE", "color": 0})
            # write strings to the file
            ...
            if n8Blocks:
                for ii in range(0, n8Blocks):
                    ...
                    fh.write(
                        "".join(
                            ["%10i" % lineN for lineN in dset["nodes"][sl : sl + 8]]
                        )
                        + "\n"
                    )
                    sl += 8
            if remLines > 0:
                fh.write(
                    "".join(["%10i" % lineN for lineN in dset["nodes"][sl:]]) + "\n"
                )
            ...
            raise UFFException(
                "The required key '"
                + msg.args[0]
                + "' not present when writing data-set #82"
            )
        except:
    ...
    def _write151(self, fh, dset):
        ...
        try:
            ...
            raise UFFException(
                "The required key '"
                + msg.args[0]
                + "' not present when writing data-set #151"
            )
        except:
    ...
    def _write164(self, fh, dset):
        ...
        try:
            # handle optional fields
            dset = self._opt_fields(
                dset, {"units_description": "User unit system", "temp_mode": 1}
            )
            # write strings to the file
            fh.write("%6i\n%6i%74s\n" % (-1, 164, " "))
            fh.write(
            ...
            raise UFFException(
                "The required key '"
                + msg.args[0]
                + "' not present when writing data-set #164"
            )
        except:
    ...
    def _write55(self, fh, dset):
        ...
        try:
            ...
            if dset["analysis_type"] == 2:
                # normal modes
                dset = self._opt_fields(
                    dset, {"modal_m": 0, "modal_damp_vis": 0, "modal_damp_his": 0}
                )
            elif dset["analysis_type"] in (3, 7):
                # complex modes
                dset = self._opt_fields(
                    dset, {"modal_b": 0.0 + 0.0j, "modal_a": 0.0 + 0.0j}
                )
                if not np.iscomplexobj(dset["modal_a"]):
                    dset["modal_a"] = dset["modal_a"] + 0.0j
                if not np.iscomplexobj(dset["modal_b"]):
                    dset["modal_b"] = dset["modal_b"] + 0.0j
                ...
                raise UFFException(
                    "Error writing data-set #55: unsupported analysis type"
                )
            ...
            raise UFFException(
                "The required key '"
                + msg.args[0]
                + "' not present when writing data-set #55"
            )
        except:
    ...
    def _write58(self, fh, dset, mode="add"):
        ...
        try:
            ...
            isEven = bool(
                dset["abscissa_spacing"]
            )  # handling even/uneven abscissa spacing manually
            ...
            fh.write(
                ...
                % (
                    ...
                    dset["abscissa_len_unit_exp"],
                    dset["abscissa_force_unit_exp"],
                    dset["abscissa_temp_unit_exp"],
                    dset["abscissa_axis_lab"],
            ...
            fh.write(
                ...
                % (
                    ...
                    dset["ordinate_len_unit_exp"],
                    dset["ordinate_force_unit_exp"],
                    dset["ordinate_temp_unit_exp"],
                    dset["ordinate_axis_lab"],
            ...
            fh.write(
                ...
                % (
                    ...
                    dset["orddenom_len_unit_exp"],
                    dset["orddenom_force_unit_exp"],
                    dset["orddenom_temp_unit_exp"],
                    dset["orddenom_axis_lab"],
            ...
            fh.write(
                ...
                % (
                    ...
                    dset["z_axis_len_unit_exp"],
                    dset["z_axis_force_unit_exp"],
                    dset["z_axis_temp_unit_exp"],
                    dset["z_axis_axis_lab"],
                ...
                if isR:
                    ...
                    if isEven:
                        ...
                        if float(n3Blocks - int(n3Blocks)) != 0.0:
                            print(
                                "Warning: Something went wrong when savning the uff file."
                            )
            ...
            raise UFFException(
                "The required key '"
                + msg.args[0]
                + "' not present when writing data-set #58"
            )
        except:
    ...
    def _write2411(self, fh, dset):
        try:
            dict = {"export_cs_number": 0, "cs_color": 8}
    ...
    def _write2420(self, fh, dset):
        try:
            dict = {"part_UID": 1, "part_name": "None", "cs_type": 0, "cs_color": 8}
            dset = self._opt_fields(dset, dict)
    ...
    def _extract18(self, blockData):
        ...
        try:
            ...
            rec_31 = [
                float(lineData[i * 13 : (i + 1) * 13])
                for i in range(int(len(lineData) / 13))
            ]
            dset["ref_o"] = np.vstack(
                (np.array(rec_31[::6]), np.array(rec_31[1::6]), np.array(rec_31[2::6]))
            ).transpose()

            # ... these are points on the x axis of cs defined in ref
            dset["x_point"] = np.vstack(
                (np.array(rec_31[3::6]), np.array(rec_31[4::6]), np.array(rec_31[5::6]))
            ).transpose()
            ...
            dset["xz_point"] = np.vstack(
                (np.array(rec_32[::3]), np.array(rec_32[1::3]), np.array(rec_32[2::3]))
            ).transpose()
        except:
    ...
    def _extract2420(self, blockData):
        ...
        dset["CS_matrices"] = [
            np.vstack((row1[i : (i + 3)], row2[i : (i + 3)], row3[i : (i + 3)]))
    ...
    def _extract151(self, blockData):
        ...
        try:
            splitData = blockData.splitlines(True)
            dset.update(
                self._parse_header_line(splitData[2], 1, [80], [1], ["model_name"])
            )
            dset.update(
                self._parse_header_line(splitData[3], 1, [80], [1], ["description"])
            )
            ...
            dset.update(
                self._parse_header_line(
                    ...
                    [10, 10],
                    [1, 1],
                    ["date_db_saved", "time_db_saved"],
                )
            )
            dset.update(
                self._parse_header_line(splitData[7], 1, [80], [1], ["program"])
            )
            dset.update(
                self._parse_header_line(
                    splitData[8],
    ...
    def _extract55(self, blockData):
        ...
        try:
            ...
            if dset["analysis_type"] == 2:
                ...
                dset.update({"modal_a": dset["modal_a_r"] + 1.0j * dset["modal_a_i"]})
                dset.update({"modal_b": dset["modal_b_r"] + 1.0j * dset["modal_b_i"]})
                dset.update({"eig": dset["eig_r"] + 1.0j * dset["eig_i"]})
                del (
                    dset["modal_a_r"],
                    dset["modal_a_i"],
                    dset["modal_b_r"],
            ...
            if dset["data_type"] == 2:
                ...
                if dset["n_data_per_node"] == 3:
                    dset["node_nums"] = values[:-6:7].copy()
                    dset["r1"] = values[1:-5:7] + 1.0j * values[2:-4:7]
                    dset["r2"] = values[3:-3:7] + 1.0j * values[4:-2:7]
                    dset["r3"] = values[5:-1:7] + 1.0j * values[6::7]
                else:
                    raise UFFException(
                        "Cannot handle 6 points per node and complex data when reading data-set #55"
                    )
            else:
    ...
    def _extract58(self, blockData):
        ...
        try:
            binary = False
            split_header = (
                b"".join(blockData.splitlines(True)[:13])
            ...
            dset.update(
                self._parse_header_line(split_header[4], 1, [80], [1], ["id3"])
            )  # usually for the date
            ...
            if binary:
                ...
                if (dset["ord_data_type"] == 2) or (dset["ord_data_type"] == 5):
                    ...
                    values = np.asarray(
                        struct.unpack(
                            "%c%sf" % (bo, int(len(split_data) / 4)), split_data
                        ),
                        "d",
                    )
                else:
                    ...
                    values = np.asarray(
                        struct.unpack(
                            "%c%sd" % (bo, int(len(split_data) / 8)), split_data
                        ),
                        "d",
                    )
            else:
                values = []
                split_data = blockData.decode("utf-8", errors="replace").splitlines(
                    True
                )[13:]
                if (dset["ord_data_type"] == 2) or (dset["ord_data_type"] == 5):
                    for line in split_data:  # '6E13.5'
                        values.extend(
                            [
                                float(line[13 * i : 13 * (i + 1)])
                                for i in range(len(line) // 13)
                            ]
                        )
                elif (
                    (dset["ord_data_type"] == 4) or (dset["ord_data_type"] == 6)
                ) and (dset["abscissa_spacing"] == 1):
                    for line in split_data:  # '4E20.12'
                        values.extend(
                            [
                                float(line[20 * i : 20 * (i + 1)])
                                for i in range(len(line) // 20)
                            ]
                        )
                    ...
                    for line in split_data:  # 1E13.5,2E20.12
                        values.extend(
                            [float(line[0:13]), float(line[13:33]), float(line[33:53])]
                        )
                else:
                    raise UFFException(
                        "Error reading data-set #58b; not proper data case."
                    )
            ...
            if (dset["ord_data_type"] == 2) or (dset["ord_data_type"] == 4):
                ...
                if dset["abscissa_spacing"] == 0:
                    ...
                    dset["data"] = values[1:-1:3] + 1.0j * values[2::3]
                else:
                    # Even abscissa
                    nVal = len(values) / 2
                    minVal = dset["abscissa_min"]
                    d = dset["abscissa_inc"]
                    dset["x"] = np.arange(minVal, minVal + nVal * d, d)
                    dset["data"] = values[0:-1:2] + 1.0j * values[1::2]
            del values
        except:
...
def prepare_test_55(save_to_file=""):
    ...
    for i, b in enumerate(modes):
        mode_shape = np.random.normal(size=node_nums)
        name = "TestCase"
        nDataPerNode = 3
        data = {
            "type": 55,
...
def prepare_test_58(save_to_file=""):
    ...
    for i, b in enumerate(binary):
        ...
        acceleration_complex = np.random.normal(
            size=len(frequency)
